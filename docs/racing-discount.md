...
Discounts are calculated in cents on the package subtotal before the existing 8% sales tax. Example: 25% off $28 = $21 + $1.68 = $22.68. The discount applies only when a new hold is created for a session on the venue’s current calendar date (VMS_VENUE_TIMEZONE, default America/New_York). Future dates use regular prices even while discount mode is on. The server compares the session start with its own current venue date; client-submitted prices and dates cannot override this rule. Public prices label the offer as today-only, and selecting a future date removes the discount from the booking preview. Paid reservations and already-created 10-minute holds retain their quoted amount. Turning the mode off affects new holds, not a customer's payment already in progress.
